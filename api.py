...
@app.get("/payment_method/{user}", response_model=list[PaymentMethod])
...
@app.patch("/payment_method/{uuid}", response_model=PaymentMethod)
def update_payment_method(uuid: UUID, payment_update: PatchPaymentMethod, session: Session = Depends(get_session)):
...
@app.delete("/payment_method/{uuid}", status_code=status.HTTP_204_NO_CONTENT)
def delete_payment_method(uuid: UUID, session: Session = Depends(get_session)):
    payment = session.get(PaymentMethod, uuid)
    if not payment:
        raise HTTPException(status_code=404, detail="Payment method not found")
